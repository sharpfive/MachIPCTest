...
int main(int argc, char **argv) {
    ...
    mach_port_t              client_port, server_port, object_handle;     // find the server
    kr = bootstrap_look_up(bootstrap_port, SERVICE_NAME, &server_port);
    EXIT_ON_MACH_ERROR("bootstrap_look_up", kr, BOOTSTRAP_SUCCESS);     // allocate a port for receiving the server's reply
    ...
    EXIT_ON_MACH_ERROR("mach_port_allocate", kr, KERN_SUCCESS);     // prepare and send a request message to the server
    ...
    do {
        ...
        object_handle = recv_msg.data.name;         { // map the specified memory object to a region of our address space
                ...
                kr = mach_vm_map(
                    ...
                    else {                 // display the current contents of the memory
                    printf("%s\n", (char *)address);
        ...
        } while (recv_hdr->msgh_id != SHM_MSG_ID);     exit(0); }
